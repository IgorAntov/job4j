...
select p.name from product p inner join type t on p.type_id = t.id where t.name = 'СЫР' or t.name = 'МОЛОКО';
--7. Написать запрос, который выводит тип продуктов, которых осталось меньше 10 штук.
select t.name, count(t.id) as cnt from product p inner join type t on p.type_id = t.id group by t.name having count(t.id) < 10;
